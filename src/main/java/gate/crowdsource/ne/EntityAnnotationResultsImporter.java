        ...
        if(unitId != null) {
          ...
          if(judgments != null) {
            for(JsonNode judgment : judgments) {
              JsonNode data = judgment.get("data");
              ArrayNode answer = (ArrayNode)data.get("answer");
              Long judgmentId = judgment.get("id").asLong();
              Double trust = judgment.get("trust").asDouble();
              Long workerId = judgment.get("worker_id").asLong();
              String comment = null;
              if(data.get("comment") != null) {
                if(data.get("comment").isTextual()) {
                  comment = data.get("comment").asText().trim();
                  if("".equals(comment)) {
                    comment = null;
                  }
                }
              ...
              if(answer != null && answer.size() > 0) {
                ...
                while(curTok < answer.size()) {
                  ...
                  if(!annotateSpans || curTok == answer.size() - 1
                          || (answer.get(curTok).asInt() + 1) != answer.get(curTok + 1).asInt()) {
                    Long startOffset = snippetTokens.get(answer.get(startTok).asInt()).getStartNode().getOffset();
                    Long endOffset = snippetTokens.get(answer.get(curTok).asInt()).getEndNode().getOffset();
                    startTok = curTok + 1;
